...
export function MfaSettings() {
	...
	return (
		<div className="space-y-4">
			<div className="flex items-start gap-3 rounded-2xl border border-neutral-200 bg-neutral-50/50 p-4 dark:border-neutral-800 dark:bg-[#18191e]">
				{status.enabled ? (
			...
			<div className="flex flex-wrap gap-3">
				{status.enabled ? (
					<>
						<Button variant="outline" onClick={() => setCodesOpen(true)} className="rounded-xl dark:border-neutral-700 dark:hover:bg-neutral-800">
							New recovery codes
						</Button>
						<Button variant="outline" onClick={() => setDisableOpen(true)} className="rounded-xl dark:border-neutral-700 dark:hover:bg-neutral-800 text-rose-600 dark:text-rose-400">
							Turn off
						</Button>
					</>
				) : (
					<Button onClick={() => setEnrollOpen(true)} className="rounded-xl bg-blue-600 hover:bg-blue-500 text-white">Turn on two-factor</Button>
				)}
			</div>
...
function RecoveryCodesList({ codes }: { codes: string[] }) {
	...
	return (
		<div className="space-y-3">
			<ul className="grid grid-cols-2 gap-2 rounded-xl border border-neutral-200 bg-neutral-50 p-4 font-mono text-xs text-neutral-800 dark:border-neutral-800 dark:bg-neutral-900 dark:text-neutral-200">
				{codes.map((code) => (
