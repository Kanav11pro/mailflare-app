const MIGRATION_NAMES = [
	...
	"0032_add_device_push_tokens.sql",
	"0033_add_privacy_masked_aliases_and_newsletters.sql",
	"0034_add_outbound_delivery_telemetry.sql",
];
...
CREATE TABLE IF NOT EXISTS masked_aliases (id text PRIMARY KEY NOT NULL, user_id text NOT NULL REFERENCES users(id) ON DELETE cascade, mailbox_id text NOT NULL REFERENCES mailboxes(id) ON DELETE cascade, domain_id text NOT NULL REFERENCES domains(id) ON DELETE cascade, local_part text NOT NULL, label text NOT NULL, status text DEFAULT 'active' NOT NULL, forward_count integer DEFAULT 0 NOT NULL, blocked_count integer DEFAULT 0 NOT NULL, last_received_at integer, expires_at integer, created_at integer NOT NULL);
CREATE UNIQUE INDEX IF NOT EXISTS masked_aliases_address_idx ON masked_aliases(domain_id, local_part);
CREATE INDEX IF NOT EXISTS masked_aliases_user_idx ON masked_aliases(user_id);
CREATE INDEX IF NOT EXISTS masked_aliases_mailbox_idx ON masked_aliases(mailbox_id);
CREATE TABLE IF NOT EXISTS device_push_tokens (id text PRIMARY KEY NOT NULL, user_id text NOT NULL REFERENCES users(id) ON DELETE cascade, token text NOT NULL UNIQUE, platform text DEFAULT 'unknown' NOT NULL, created_at integer NOT NULL, updated_at integer NOT NULL);
