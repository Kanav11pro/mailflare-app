...
import { domains, folders, mailboxAliases, mailboxes, routingRules, maskedAliases } from "@/db/schema";
...
export async function resolveInboundAddress(
	...
	// Phase 2 — a real mailbox or active masked alias always wins over a catch-all.
	const maskedRow = await resolveMaskedAlias(db, domain.id, parsed.localPart);
	if (maskedRow) {
		const { maskedAlias, mailbox: parentMailbox } = maskedRow;
		const isExpired = maskedAlias.expiresAt && new Date(maskedAlias.expiresAt).getTime() < Date.now();
		if (maskedAlias.status === "killed" || isExpired) {
			await db
				.update(maskedAliases)
				.set({ blockedCount: maskedAlias.blockedCount + 1 })
				.where(eq(maskedAliases.id, maskedAlias.id));
			return {
				action: "reject",
				rejectReason: isExpired ? "Masked burner alias has expired" : "Masked alias has been disabled",
			};
		}
		if (maskedAlias.status === "paused") {
			await db
				.update(maskedAliases)
				.set({ blockedCount: maskedAlias.blockedCount + 1 })
				.where(eq(maskedAliases.id, maskedAlias.id));
			return {
				action: "reject",
				rejectReason: "Masked alias is temporarily paused",
			};
		}
		...
		await db
			.update(maskedAliases)
			.set({
				forwardCount: maskedAlias.forwardCount + 1,
		...
		return {
			action: "store",
			mailbox: toResolvedMailbox(parentMailbox, domain.id, domain.hostname),
		};
	}
...
async function resolveMaskedAlias(
	db: AppDatabase,
	domainId: string,
	localPart: string,
) {
	const [row] = await db
		.select({
			maskedAlias: maskedAliases,
			mailbox: mailboxes,
		})
		.from(maskedAliases)
		.innerJoin(mailboxes, eq(maskedAliases.mailboxId, mailboxes.id))
		.where(
			and(
				eq(maskedAliases.domainId, domainId),
				eq(maskedAliases.localPart, localPart),
			),
		)
		.limit(1);
